    ...
    const handlePrint = async () => {
        ...
        try {
            ...
            if (printer.isUsb) {
                ...
                result = await window.electronAPI.printToUsb(selectedPrinterId, selectedFile.path, copies);
            }
            else {
                // Отправляем на сетевой принтер
                result = await window.electronAPI.sendToPrinter(selectedFile.path, printer.ipAddress ?? '', // добавляем проверку на undefined
        ...
        catch (err) { // Указываем тип для ошибки
